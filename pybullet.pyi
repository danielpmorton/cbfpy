"""Python bindings for Bullet Physics Robotics API

Note: these bundings were autogenerated and last updated in January 2022
These are not guaranteed to be the exact bindings - refer to the pybullet docs for
more precise information

File from https://github.com/rohit-kumar-j/temporary_pybullet_stub/blob/main/pybullet.pyi
...
def addUserData(
    bodyUniqueId: int, key, value, linkIndex=-1, visualShapeIndex=-1, physicsClientId=0
...
def addUserDebugLine(
    ...
    lineToXYZ: list[float],
    lineColorRGB: list[float],
    lineWidth: float,
    lifeTime: float,
...
def addUserDebugText(
    ...
    textColorRGB: list[float],
    textSize: float,
    lifeTime: float,
    textOrientation: list[float],
    parentObjectUniqueId: int,
    parentLinkIndex: int,
    replaceItemUniqueId: int,
    physicsClientId: int,
...
def calculateInverseDynamics(
    bodyUniqueId: int,
    objPositions: list[float],
    objVelocities: list[float],
...
def calculateInverseKinematics(
    ...
    targetOrientation: list[float],
    lowerLimits: list[float],
    upperLimits: list[float],
    jointRanges: list[float],
    restPoses: list[float],
    jointDamping: list[float],
    solver: int,
    currentPosition: list[float],
...
def calculateInverseKinematics2(
    bodyUniqueId: int,
    endEffectorLinkIndices: list[int],
...
def changeDynamics(
    ...
    frictionAnchor: int,
    localInertiaDiagnoal: list,
    ccdSweptSphereRadius: float,
    contactProcessingThreshold: float,
    activationState: int,
    jointDamping: float,
    anisotropicFriction: float,
    maxJointVelocity: float,
...
def createMultiBody(
    ...
    baseOrientation: list[float],
    baseInertialFramePosition: list[float],
    baseInertialFrameOrientation: list[float],
    linkMasses: list[float],
    linkCollisionShapeIndices: list[int],
    linkVisualShapeIndices: list[int],
    linkPositions: list[float],
    linkOrientations: list[float],
    linkInertialFramePositions: list[float],
    linkInertialFrameOrientations: list[float],
    linkParentIndices: list[int],
    linkJointTypes: list[int],
    linkJointAxis: list[float],
...
def loadSoftBody(
    ...
    collisionMargin,
    useMassSpring: bool,
    useBendingSprings: bool,
    useNeoHookean: bool,
    springElasticStiffness: float,
    springDampingStiffness: float,
    springDampingAllDirections: float,
    springBendingStiffness: float,
    NeoHookeanMu: float,
    NeoHookeanLambda: float,
    NeoHookeanDamping: float,
    frictionCoeff: float,
    useFaceContact: bool,
...
def loadURDF(
    fileName: str,
    basePosition: list[float] = None,
    baseOrientation: list[float] = None,
...
def multiplyTransforms(
    ...
    orientationA: list[float],
    positionB: list[float],
...
def setJointMotorControlArray(
    ...
    forces: list[float],
    positionGains: list[float],
    velocityGains: list[float],
...
def setVRCameraState(
    ...
    rootOrientation: list[float],
    trackObject: list[float],
    trackObjectFlag: int,
    physicsClientId: int = 0,
    *args,
    **kwargs
